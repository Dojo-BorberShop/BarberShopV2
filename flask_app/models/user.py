...
# for validation messages, which will appear  after selecting submit.The moment we clcik somewhere else or click or refresh, the message dissapears, which is why it's called flash.
...
class User:
    ...
    def __init__(self, data_dictionary):  # data_dictionary is a dictionary from your database
        ...
        self.appointments = []  # Place holder for holding MANY recipes
    ...
    def register_user(cls, data):
        query = """
        INSERT INTO users (is_owner, first_name, last_name, cell, email, password)
        VALUES (%(is_owner)s, %(first_name)s, %(last_name)s, %(cell)s, %(email)s, %(password)s)
        """
    ...
    def grab_one_user_by_id(cls, data):
        ...
        if len(results) == 0:
            ...
            # Create a user object from the results list at index 0.
    ...
    def validate_registration(form_data):
        ...
        if len(form_data["password"]) < 8:
            ...
            is_valid = False
        if 'is_owner' not in form_data or form_data['is_owner'] == "":
            flash("Must declare if you are a business owner", "registration")
            is_valid = False
        ...
        Found_User_Or_None = User.grab_one_user_by_email(
        ...
        if Found_User_Or_None != None:
            flash("Email invalid.", "registration")
            is_valid = False
    ...
    def validate_login(form_data):
        ...
        if Found_User_Or_None == None:  # There's no need to check the password.
            # Do not givr a specific error message like invalid email or invalid password. This could let a hacker know that they got the correct email but just need to hack into the password or vice versa. We don't want hackers to know whether the email or the password was wrong or both.
            flash("Invalid login credentials", "login")
            is_valid = False
            # no need to check password if no user found, so add return statement to stop validation.
            return False
            # If someone exists with this email, then check the password.
        # Found_User_Or_None returns an object (see grab_one_user_by_email method, where it makes an object) if the emails match. Then, we look at the object and see if the email entered in the loggin form matches the password in the object (which include the correct password for the given email address).
